...
- **验收标准**：不破坏现有 PNG 动作；待机状态可通过 rig 呈现呼吸、眨眼、头发摆动和鼠标注视；大动作播放前后能自然切回 rig 或现有停留帧。
...
| `hairSway` | `frameClip` | 机械拆层头发边缘和遮挡不够干净，先回退 PNG。 |
| `blink` / `slowBlink` | `frameClip` 兼容枚举 | 机械生成的闭眼覆盖层会在脸部产生线条，运行素材已清理，默认不调度。 |
| `eyeShiftLeft` / `eyeShiftRight` | `frameClip` 兼容枚举 | 眼睛/眼镜没有干净拆层前不保留运行素材，默认不调度。 |
| `cursorLook` | `frameClip` | 头眼联动需要高质量脸部拆层，先回退 PNG。 |
| `shoulderRelax` / `tinyHandAdjust` | `frameClip` | 第一版保守处理，避免真人手臂拆层穿帮。 |
...
| 状态 | 说明 | 进入方式 |
|------|------|----------|
| `rigIdle` | rig 可见，播放待机呼吸/眨眼等循环 | 展示状态 settle，且 rig 可用。 |
...
| 阶段 | 内容 | 产物 | 验收 |
|------|------|------|------|
| Phase 1 | 最小 rig PoC | `lingxi-ol-rig` 最小拆层、`PetRigProvider`、`PetRigView` | 待机呼吸、眨眼可见；PNG fallback 正常。 |
| Phase 2 | 接入动作分流 | `PetRenderModePolicy`、`PetView.play` 分流 | `breathing`、`hairSway`、`cursorLook` 走 rig；大动作仍走 PNG。 |
...
截至 2026-05-20，Phase 1 已进入最小可运行实现：
...
- 当前 rig 仅支持 `breathing`。脸部和头发相关 rig 动作已回退 PNG，因为机械拆层会产生脸部覆盖线和重复叠图。后续必须重新生成干净的脸部/头发/眼镜拆层素材后再打开。
